...
function victorySubtitle(result: Victory, players: ResultSnapshot["players"]): string {
  switch (result.type) {
    case "ONGOING":
      return "";
    case "CITIZEN":
      return "Tous les Imposteurs et les Inconnus ont été éliminés.";
    case "IMPOSTOR":
      return "Au moins un Imposteur a survécu jusqu'à la fin.";
    case "COMBINED": {
      ...
      return `Les Citoyens sont éliminés : ${impostorLabel} et ${unknownLabel} gagnent ensemble.`;
    }
    case "UNKNOWN":
      if (result.byGuess) {
        const winner = players.find((p) => p.playerId === result.winnerIds[0]);
        return `${winner?.pseudo ?? "l'Inconnu"} a deviné le mot exact !`;
      }
      return "l'Inconnu a survécu jusqu'à la fin.";
  }
}
...
export function renderResult(state: AppState, actions: Actions): HTMLElement {
  ...
  screen.append(
    h(
      ...
      h("div", { class: "spacer" }),
      h("div", { class: "center" }, scrim(victorySubtitle(result.victory, result.players))),
