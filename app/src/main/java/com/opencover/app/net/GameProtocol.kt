            ...
            for (i in 0 until arr.length()) {
                ...
                add(
                    Player(
                        ...
                        role = parseRole(p.optString("role")) ?: Role.CIVIL,
                        assignedWord = ""
                    )
                )
            }
...
private fun Victory.toWire(): Triple<String, Set<Int>, Boolean> = when (this) {
    ...
    Victory.Civil -> Triple("CIVIL", emptySet(), false)
    Victory.Undercover -> Triple("UNDERCOVER", emptySet(), false)
    is Victory.MrWhite -> Triple("MRWHITE", winnerIds, byGuess)
...
private fun victoryFromWire(type: String, winnerIds: Set<Int>, byGuess: Boolean): Victory = when (type) {
    "CIVIL" -> Victory.Civil
    "UNDERCOVER" -> Victory.Undercover
    "MRWHITE" -> Victory.MrWhite(winnerIds, byGuess)
